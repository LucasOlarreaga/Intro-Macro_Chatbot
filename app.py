...
# ---------------------------------------------------------------------------
# Secrets — works with .streamlit/secrets.toml locally and env vars on Railway
# ---------------------------------------------------------------------------
def get_secret(key: str) -> str:
    try:
        ...
        if val is None:
            raise KeyError(f"Secret '{key}' not found. Set it in .streamlit/secrets.toml or as an environment variable.")
        return val
...
# ---------------------------------------------------------------------------
def stream_response(client, system: str, messages: list):
...
def check_password():
    ...
    if controller.get("macro_auth") == get_secret("COOKIE_TOKEN"):
    ...
    if st.button(btn_label, disabled=not acknowledged):
        if password == get_secret("APP_PASSWORD"):
            controller.set("macro_auth", get_secret("COOKIE_TOKEN"), max_age=7 * 24 * 60 * 60)
            st.session_state["authenticated"] = True
            st.rerun()
        else:
